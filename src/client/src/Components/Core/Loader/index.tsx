...
export default function Loader({
  ...
  return (
    <div className={`relative w-96 h-64 rounded-md ${playerColor}`}>
      ...
      <div className="absolute top-3 left-3 bg-neutral-800 w-8 h-8 rounded-full" />
      <div className="absolute bottom-3 left-3 bg-neutral-800 w-8 h-8 rounded-full" />

      {/* Vinyl Record */}
      <VinylRecord color={vinylColor} size={size} onPlayer />

      {/* Needle */}
      <div className="absolute bg-neutral-800 h-12 w-12 rounded-full top-2 right-16 animate-wiggle">
        <div className="absolute bg-neutral-900 h-4 w-4 rounded-full top-4 right-4" />
      ...
      {/* Stop Button */}
      <div className="absolute bg-neutral-800 h-10 w-10 bottom-3 right-6 rounded-full" />

      {/* Volume Control */}
      <div className="absolute bg-neutral-900 h-32 w-10 bottom-16 right-6 rounded-md">
        <div className="absolute bg-neutral-800 w-1 h-28 top-2 left-[18px] rounded-sm" />
        <div className="absolute bg-neutral-700 w-8 h-4 bottom-8 left-1 rounded-sm" />
      </div>
    </div>
  );
}
...
function VinylRecord({ onPlayer, color = 'random', size = 'lg' }: VinylProps) {
  ...
  return (
    <div
      className={`relative bg-neutral-800 rounded-full ${recordSize} ${playerProps}`}
    >
      <div
        className={`relative rounded-full ${centerColoredSize} ${colorProps}`}
      >
        <div
          className={`absolute bg-neutral-900 rounded-full ${centerDotSize}`}
        />
      </div>
      <div
